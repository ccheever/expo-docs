...
If the user cancelled the image picking, returns ${<InlineCode>{`{ cancelled: true }`}</InlineCode>}.

Otherwise, returns ${<InlineCode>{`{ cancelled: false, uri, width, height, exif, base64 }`}</InlineCode>} where \`uri\` is a URI to the local image file (useable as the source for an \`Image\` element) and \`width, height\` specify the dimensions of the image. \`base64\` is included if the \`base64\` option was truthy, and is a string containing the JPEG data of the image in Base64--prepend that with \`'data:image/jpg;base64,'\` to get a data URI, which you can use as the source for a\`Image\` element for example. \`exif\` is included if the \`exif\` option was truthy, and is an object containing EXIF data for the image--the names of its properties are EXIF tags and their values are the values for those tags.
...
If the user cancelled taking a photo, returns ${<InlineCode>{`{ cancelled: true }`}</InlineCode>}.
