...
Unless \`Sentry.enableInExpoDevelopment = true\` is set before calling ${<InlineCode>{'Sentry.config({...}).install()'}</InlineCode>}, all your dev/local errors will be ignored and only app releases will report errors to Sentry. You can call methods like \`Sentry.reportException\` but these methods will be no-op.
