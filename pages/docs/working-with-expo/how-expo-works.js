...
${
  <Image
    src={`${ASSETS_URL}/docs/guides/fetch-app-from-xde.png`}
    width={2152}
    height={696}
  />
}
...
The second purpose is to serve assets. When you include an image in your app, you will use syntax like ${<InlineCode>{"<Image source={require('./assets/example.png')} />"}</InlineCode>}, unless you have already cached that asset you will see a request in the XDE logs like: \`<START> processing asset request my-proejct/assets/example@3x.png\`. Notice that it serves up the correct asset for your screen DPI, assuming that it exists.

## Publishing/Deploying an Expo app in Production

When you Publish an Expo app, we compile it into a JavaScript bundle with production flags enabled (minify, disable runtime development checks) and upload that bundle, along with any assets that it requires (see [Assets](assets#all-about-assets)) to CloudFront. We also upload your [Manifest](#expo-manifest) (including most of your \`app.json\` configuration) to our server.
