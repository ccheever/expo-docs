...
-   ${<InlineCode>{`Expo.Audio.Sound.create(source, initialStatus = {}, onPlaybackStatusUpdate = null, downloadFirst = true)`}</InlineCode>}
    ...
    -   **source (_object_ / _number_ / _Asset_)** -- The source of the sound. The following forms are supported:
        ...
        -   A dictionary of the form ${<InlineCode>{`{ uri: 'http://path/to/file' }`}</InlineCode>} with a network URL pointing to an audio file on the web.
        -   \`require('path/to/file')\` for an audio file asset in the source code directory.
        -   An [\`Expo.Asset\`](asset.html) object for an audio file asset.

    -   **initialStatus (_PlaybackStatusToSet_)** -- The initial intended \`PlaybackStatusToSet\` of the sound, whose values will override the default initial playback status. This value defaults to ${<InlineCode>{`{}`}</InlineCode>} if no parameter is passed. See the [AV documentation](av.html) for details on \`PlaybackStatusToSet\` and the default initial playback status.
...
-   ${<InlineCode>{`soundObject.loadAsync(source, initialStatus = {}, downloadFirst = true)`}</InlineCode>}
