...
${
  <Image
    src={`${ASSETS_URL}/docs/sdk/facebook-app-settings.png`}
    width={800}
    height={602}
  />
}
...
-   **Android standalone app**
    ...
    -   Run ${<InlineCode>{`keytool -list -printcert -jarfile YOUR_APK.apk | grep SHA1 | awk '{ print $2 }' | xxd -r -p | openssl base64`}</InlineCode>} (replace \`YOUR_APK.apk\` with the name of your APK file).
...
If the user or Facebook cancelled the login, returns ${<InlineCode>{`{ type: 'cancel' }`}</InlineCode>}.

Otherwise, returns ${<InlineCode>{`{ type: 'success', token, expires }`}</InlineCode>}. \`token\` is a string giving the access token to use with Facebook HTTP API requests. \`expires\` is the time at which this token will expire, as seconds since epoch. You can save the access token using, say, \`AsyncStorage\`, and use it till the expiration time.
...
Given a valid Facebook application ID in place of ${<InlineCode>{`<APP_ID>`}</InlineCode>}, the code above will prompt the user to log into Facebook then display the user's name. This uses React Native's [fetch](https://facebook.github.io/react-native/docs/network.html#fetch) to query Facebook's [Graph API](https://developers.facebook.com/docs/graph-api).
