...
${<InlineCode>{"import { Ionicons } from '@expo/vector-icons';"}</InlineCode>} instead of.. \`import Ionicons from 'react-native-vector-icons/Ionicons';\`.

> **Note:** As with [any custom font](using-custom-fonts.html#using-custom-fonts) in Expo, you may want to preload icon fonts before rendering your app. The font object is available as a static property on the font component, so in the case above it is \`Ionicons.font\`, which evaluates to ${<InlineCode>{"{ionicons: require('path/to/ionicons.ttf')}"}</InlineCode>}. [Read more about preloading assets](./assets#all-about-assets).
