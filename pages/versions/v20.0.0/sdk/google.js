...
If the user or Google cancelled the login, returns ${<InlineCode>{`{ type: 'cancel' }`}</InlineCode>}.

Otherwise, returns ${<InlineCode>{`{ type: 'success', accessToken, idToken, refreshToken, {...profileInformation} }`}</InlineCode>}, \`accessToken\` is a string giving the access token to use with Google HTTP API requests.
...
-   **Get a Google API Key for your app** (_skip this if you already have one, eg: for Google Maps_)
   ...
   6.  Run ${<InlineCode>{`keytool -list -printcert -jarfile growler.apk | grep SHA1 | awk '{ print $2 }'`}</InlineCode>} (where \`growler.apk\` is the name of the apk produced in step 1).
...
-   **Get an OAuth client ID for your app**
   ...
   4.  Run ${<InlineCode>{`keytool -list -printcert -jarfile growler.apk | grep SHA1 | awk '{ print $2 }'`}</InlineCode>} (where \`growler.apk\` is the name of the apk produced in step 1).
...
-   **Add the configuration to your app**
   ...
   4.  Run ${<InlineCode>{`keytool -list -printcert -jarfile growler.apk | grep SHA1 | awk '{ print $2 }' | sed -e 's/\://g'`}</InlineCode>} (where \`growler.apk\` is the name of the apk produced in step 1).
